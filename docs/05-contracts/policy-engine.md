...
> **接线状态（2026-08-24 修订）**：本条是设计意图，
> 但 v0.1 实现中 `post_develop` 判定点**未挂 EvaluatePolicy 副作用**
> （转移表只有 `rfc_ready` / `capability_request` 接线）——
> 写了也没人读的规则已从 `DEFAULT_RULES` 删除，不假装接线。
> 2026-08-25（issue #23 R6）：对应的 `loadPolicyFacts` 加载分支与
> `FACTS_AT` 条目一并删除 —— 未接线判定点连「加载」入口都没有,
> validate 对引用未接线点的规则直接报错。
> 接入漂移检测时：补 `run(develop)` 成功后的转移 + 恢复规则与加载分支。
