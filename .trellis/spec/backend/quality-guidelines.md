...
| `KEEL_HARNESS` | 选择 Adapter(`omp` / `claude`;CLI `--harness` 优先) | 可选；缺省 `omp`；非法/空白拒绝、不静默回退 |
| `ANTHROPIC_API_KEY` | Claude Code `--bare` 认证(不读 OAuth / 钥匙串) | `--harness claude` 与 `claude-code-e2e` 必须 |
